 ...
 *
 * ⚠ PROCEDENCIA DE LA MUESTRA: es SINTÉTICA, no una captura de producción. Se
 *   intentó capturar cotizaciones reales el 8 de septiembre de 2026, pero las
 *   seis consultas de detalle devolvieron HTTP 504 y no hubo nada que guardar.
 *   Lo que sí reproduce fielmente es la FORMA que devuelve la API, verificada
 *   contra respuestas reales: precio_unitario dentro de productos_cotizados[],
 *   valor_neto y monto_total a nivel raíz del proveedor, nulos frecuentes, y
 *   casi todas las cotizaciones declaradas inadmisibles (es lo que deja
 *   desierto al proceso).
 *
 *   Por eso este archivo verifica la ARITMÉTICA, que es determinista y no
 *   depende de la API. Que la muestra real se parezca a esta sigue pendiente
 *   de comprobar cuando el servicio esté sano.
 */
